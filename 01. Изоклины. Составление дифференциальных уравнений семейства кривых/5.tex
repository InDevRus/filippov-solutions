...
Итак,\, $f\br{x; y} = -\dfrac {x} {y}$.
Уравнение изоклины: 
$-\dfrac {x} {y} = k$. $x = -ky$. 
Таким образом, изоклинами будут прямые. 
Неформально говоря, при $y = 0$, $x \ne 0$ касательная будет строго вертикальной. 
В точке $x = 0$, $y = 0$, однако, поведение касательной остаётся неизвестным. 
