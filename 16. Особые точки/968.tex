...
$\det\br{\lambda E - A} = \br{\lambda - 1}\br{\lambda - 4} + 2 
= \pow{\lambda}{2}  - 5\lambda + 6$.
$\sub{\lambda}{1} = 2$, $\sub{\lambda}{2} = 3$.
Особая точка является <<узлом>>.

$A - \sub{\lambda}{1}E =
\begin{pmatrix} -1 & 1 \\ -2 & 2 \end{pmatrix}
\sim \begin{pmatrix} -1 & 1 \end{pmatrix}$, $\sub{v}{1} 
= \begin{pmatrix} 1 \\ 1 \end{pmatrix}$;
$A - \sub{\lambda}{2}E =
\begin{pmatrix} -2 & 1 \\ -2 & 1 \end{pmatrix}
\sim \begin{pmatrix} -2 & 1 \end{pmatrix}$,
$\sub{v}{2} = \begin{pmatrix} 1 \\ 2 \end{pmatrix}$.
$\sub{\lambda}{1}$ по модулю меньше, а потому прямая $\sub{y}{1} = x$
будет являться касательной для всех траекторий (частных решений) в начале координат, кроме
$\sub{y}{2} = 2x$.
...
$\tsys{
    x = \pm \ f\br{\p} \\
    y = \pm \,p f\br{\p}
}$, где $f\br{\p} = C \dfrac {\br{\p - 1}^2} {\br{\p - 2}^3}$.
