\documentclass[a5paper,10pt]{article}

% https://github.com/InDevRus/filippov-solutions
\input{../preambles/general}

\begin{document}

\begin{framed}
    \begin{proposition}\end{proposition}
    При $r\br{\xi; \phi} = \sqrt{\pow{\xi}{2} + \pow{\phi}{2}} \to 0$ имеем $\xi \phi = O\br{\pow{r}{2}}$. В частности, $\dfrac{\xi \phi} {\pow{r}{1 + \epsilon}} = \dfrac {\gamma\br{r} \pow{r}{2}} {\pow{r}{1 + \epsilon}} \to 0$ для всех $\epsilon \in \br{0; 1}$.
    
    Действительно, 
    $0 \le \br{\vbr{\xi} - \vbr{\phi}}^2$
    и 
    $\vbr{\xi \phi} 
    \le \dfrac {1} {2} \br{\pow{\xi}{2} + \pow{\phi}{2}}
    \le \dfrac {1} {2} \br{\pow{r}{2} + \pow{r}{2}}
    = \pow{r}{2}$,
    что показывает $\xi \phi = O\br{\pow{r}{2}}$.
\end{framed}

$\tsys{
    & P\br{x; y} = \ln\br{1 - y + \pow{y}{2}}
    \\& Q\br{x; y} = 3 - \sqrt{\pow{x}{2} + 8y}
}$;
$\tsys{
    & \pow{y}{2} - y = 0
    \\& \pow{x}{2} = 9 - 8y
}$.
Итак, получаем 4 особые точки: $A_{1}\br{-3; 0}$, \linebreak $A_{2}\br{-1; 1}$, $A_{3}\br{1; 1}$, $A_{4}\br{3; 0}$.

Для оптимизации процесса разложим каждую функцию заранее. Далее будут применены замены вида $\tsys{& x = \xi_{i} + x_{i} \\& y = \phi_{i} + y_{i}}$, где $A_{i}\br{x_{i}; y_{i}}$, $1 \le i \le 4$. Предположим, что $\xi_{i} \to 0$ и $\phi_{i} \to 0$. Тогда $\pow{y}{2} - y \to 0$ и $\pow{x}{2} + 8y \to 9$, $O\br{\br{x - x_{i}}^k} = O\br{\xi_{i}^k} = O\br{\br{O\br{r_{i}}}^k} = O\br{r_{i}^k}$ для натуральных $k$, и аналогично для $y - y_{i}$ и $\phi_{i}$.
\begin{itemize}
    \item $\ln\br{1 - y + \pow{y}{2}}
    = -y + \pow{y}{2} + O\br{\br{-y + \pow{y}{2}}^2} = -y + \pow{y}{2} + O\br{r_{i}^2}
    = -\phi_{i} - y_{i} + \pow{\phi}{2}_{i} + 2\phi_{i}y_{i} + \linebreak + \pow{y}{2}_{i} + O\br{r_{i}^2}
    = \br{\pow{y}{2}_{i} - y_{i}} + \br{2y_{i} - 1}\phi_{i} + O\br{r_{i}^2}
    \vspace{1mm} = \br{2y_{i} - 1}\phi_{i} + O\br{r_{i}^2}$, так как $\pow{y}{2}_{i} - y_{i} = 0$;
    
    \item $3 - \sqrt{\pow{x}{2} + 8y}
    = 3 - 3\sqrt{1 + \br{\dfrac {1} {9} \br{\pow{x}{2} + 8y} - 1}}
    = - 3\br{1 + \dfrac {1}{2} \br{\dfrac {1}{9} \br{\pow{x}{2} + 8y} - 1} + O\br{r_{i}^2}} \linebreak + 3 = -\dfrac {1}{6} \br{\pow{x}{2} + 8y} + \dfrac {3} {2} + O\br{r_{i}^2}
    = -\dfrac {1}{6} \br{\pow{x}{2}_{i} + 2\xi_{i} x_{i} + \pow{\xi}{2}_{i} + 8\phi_{i} + 8y_{i} } + \dfrac {3} {2} + O\br{r_{i}^2} \linebreak
    = -\dfrac {1}{6} \br{\br{\pow{x}{2}_{i} + 8y_{i}} + 2\xi_{i} x_{i} + 8\phi_{i} } + \dfrac {3} {2} + O\br{r_{i}^2}
    \vspace{1mm} = -\dfrac {1} {3} x_{i} \xi_{i} - \dfrac {4} {3} \phi_{i} + O\br{r_{i}^2}$, поскольку $\pow{x}{2}_{i} + 8y_{i} = 9$. \vspace{1mm}
\end{itemize}

Итак, после всякой замены 
$\tsys{& x = \xi_{i} + x_{i} \\& y = \phi_{i} + y_{i}}$
имеем $\tsys{& \dot{\xi}\br{t} = \br{2y_{i} - 1}\phi_{i} + O\br{r_{i}^2} \\& \dot{\phi}\br{t} = -\dfrac {1} {3} x_{i} \xi_{i} - \dfrac {4} {3} \phi_{i} + O\br{r_{i}^2}}$.

\begin{enumerate}
    \item $\tsys{& x_{1} = -3 \\& y_{1} = 0}$; $\tsys{& x = \xi - 3 \\& y = \phi}$.
    $\tsys{& \dot{\xi} = -\phi + O\br{\pow{r}{2}} \\& \dot{\phi} = \xi - \dfrac {4} {3} \phi + O\br{\pow{r}{2}}}$.
    $A = \begin{pmatrix} 0 &  -1 \\ 1 & -\frac {4} {3} \end{pmatrix}$.
    $\pow{\lambda}{2} - \dfrac {4} {3} \lambda + 1 = 0$.
    $\lambda_{1, 2} = \linebreak = \dfrac {-2 \mp \sqrt{5}i} {3}$, особая точка $A_{1}\br{-3; 0}$ -- <<фокус>>, причём устойчивый.
    
    Система до линеаризации выглядит, как
    $\tsys{& \dot{\xi} = \ln\br{1 + \pow{\phi}{2} - \phi} \\& \dot{\phi} = 3 - \sqrt{\pow{\xi}{2} - 6\xi + 8\phi + 9}}$ и $\tsys{& \dot{\xi}\br{\frac {1} {4}; 0} = 0 \\& \dot{\phi}\br{\frac {1} {4}; 0} = \frac {1} {4}}$, то есть направления закручивания -- против часовой.
    
    \item $\tsys{& x_{2} = -1 \\& y_{2} = 1}$; $\tsys{& x = \xi - 1 \\& y = \phi + 1}$.
    $\tsys{& \dot{\xi} = \phi + O\br{\pow{r}{2}} \\& \dot{\phi} = \dfrac {1} {3} \xi - \dfrac {4} {3} \phi + O\br{\pow{r}{2}}}$.
    $A = \begin{pmatrix} 0 &  1 \\ \frac {1} {3} & -\frac {4} {3} \end{pmatrix}$.
    $\pow{\lambda}{2} + \dfrac {4} {3} \lambda - \dfrac {1} {3} = 0$.
    $\lambda_{1, 2} = \dfrac {-2 \mp \sqrt{7}} {3}$, особая точка $A_{2}\br{-1; 1}$ -- <<седло>>.
    $v_{1} = \begin{pmatrix} -3 \\ 2 + \sqrt{7}\end{pmatrix}$, $v_{2} = \begin{pmatrix} -3 \\ 2 - \sqrt{7}\end{pmatrix}$, а $\phi = \dfrac {-2 -\sqrt{7}} {3} \xi$ и $\phi = \dfrac {-2 +\sqrt{7}} {3} \xi$ -- сепаратрисы.
    
    \item $\tsys{& x_{3} = 1 \\& y_{3} = 1}$; $\tsys{& x = \xi + 1 \\& y = \phi + 1}$.
    $\tsys{& \dot{\xi} = \phi + O\br{\pow{r}{2}} \\& \dot{\phi} = -\dfrac {1} {3} \xi - \dfrac {4} {3} \phi + O\br{\pow{r}{2}}}$.
    $A = \begin{pmatrix} 0 &  1 \\ -\frac {1} {3} & -\frac {4} {3} \end{pmatrix}$.
    $\pow{\lambda}{2} + \dfrac {4} {3} \lambda + \dfrac {1} {3} = 0$.
    $\lambda_{1} = -1$, $\lambda_{2} = -\dfrac {1} {3}$, особая точка $A_{3}\br{1; 1}$ -- <<узел>>, притом устойчивый.
    
    $A - \lambda_{1} E 
    = \begin{pmatrix} 1 & 1 \\ -\frac {1} {3} & -\frac {1} {3} \end{pmatrix}
    \sim \begin{pmatrix} 1 & 1 \end{pmatrix}$.
    $v_{1} = \begin{pmatrix} 1 \\ -1 \end{pmatrix}$.
    $A - \lambda_{2} E 
    = \begin{pmatrix} \frac {1} {3} & 1 \\ -\frac {1} {3} & 1 \end{pmatrix}
    \sim \begin{pmatrix} 1 & 3 \end{pmatrix}$.
    $v_{2} = \begin{pmatrix} 3 \\ -1 \end{pmatrix}$.
    Следовательно, $\phi = -\xi$ -- трансверсаль, $\phi = -\dfrac {1} {3} \xi$ -- касательная.
    
    \item $\tsys{& x_{4} = 3 \\& y_{4} = 0}$; $\tsys{& x = \xi + 3 \\& y = \phi}$.
    $\tsys{& \dot{\xi} = -\phi + O\br{\pow{r}{2}} \\& \dot{\phi} = -\xi - \dfrac {4} {3} \phi + O\br{\pow{r}{2}}}$.
    $A = \begin{pmatrix} 0 & -1 \\ -1 & -\frac {4} {3} \end{pmatrix}$.
    $\pow{\lambda}{2} + \dfrac {4} {3} \lambda - 1 = 0$.
    $\lambda_{1, 2} = \linebreak = \dfrac {-2 \mp \sqrt{13}} {3}$, особая точка $A_{4}\br{3; 0}$ -- <<седло>>.
    $v_{1} = \begin{pmatrix} 3 \\ 2 + \sqrt{13} \end{pmatrix}$, $v_{2} = \begin{pmatrix} 3 \\ 2 - \sqrt{13} \end{pmatrix}$.
    
    $\phi = \dfrac {\sqrt{13} + 2} {3} \xi$, $\phi = \dfrac {\sqrt{13} - 2} {3} \xi$ -- сепаратрисы.
\end{enumerate}

\end{document}
