...
$\dfrac {\d y} {\d x} = \dfrac {2x - 2\pow{x}{3}} {y}$.
...
\begin{enumerate}
    \item $K\br{0; 0}$. $\dfrac {\d y} {\d x} = \dfrac {2x - 2\pow{x}{3}} {y} = \dfrac {2x + O\br{\pow{x}{3}}} {y}$.
    ...
    Для первой имеем $\dfrac {\d y} {\d x} = \dfrac {-4\br{x + 1} + 6\br{x + 1}^2 - 2\br{x + 1}^3} {y} = \dfrac {-4\br{x + 1} + O\br{x + 1}^2} {y}$,
    ...
    $\dfrac {\d y} {\d x} = \dfrac {-4\br{x - 1} - 6\br{x - 1}^2 - 2\br{x - 1}^3} {y} = \linebreak =
    \dfrac {-4\br{x - 1} + O\br{x - 1}^2} {y}$.

    Для обеих матрица оператора равна $\begin{pmatrix} 0 & 1 \\ -4 & 0 \end{pmatrix}$, $\pow{\lambda}{2} + 4 = 0$. Из-за обозначенной выше симметрии обе особые точки -- <<центры>>.
\end{enumerate}
...
\pagebreak
Построенный фазовый портрет позволяет сделать вывод о поведении кривых при $t \to +\infty$. Пусть $x\br{t}$ -- решение задачи Коши $\ddot{x}\br{t} = 2x - 2\pow{x}{3}$, $x\br{t_0} = a$, $\dot{x}\br{t_0} = b$. Обозначим $\pow{b}{2} + \br{\pow{a}{2} + 1}^2 = C \ge 0$. При $C \ne 1$ любое решение $x\br{t}$ при $t \to +\infty$ принимает ограниченный повторяющийся набор значений, причём:
\begin{itemize}
    \item если $\pow{b}{2} + \br{\pow{a}{2} - 1}^2 = C \in \br{0; 1}$, то решения сохраняют знак и находятся в окрестности особой точки:
    \begin{itemize}
        \item при $a > 0$: $0 < \sqrt{1 - \sqrt{C}} \le x\br{t} \le \sqrt{1 + \sqrt{C}}$;
        \item при $a < 0$: $-\sqrt{1 + \sqrt{C}} \le x\br{t} \le -\sqrt{1 - \sqrt{C}} < 0$;
    \end{itemize}

    \item если $\pow{b}{2} + \br{\pow{a}{2} - 1}^2 = C > 1$, то решения знакопеременны, проходят через ноль бесконечное число раз, и
