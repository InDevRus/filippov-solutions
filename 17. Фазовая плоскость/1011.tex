...
$\der{y}{x} = \dfrac {-3x + 4y} {y}$.

Особой точкой является $M\br{0; 0}$.
$\begin{pmatrix} 0 & 1 \\ -3 & 4 \end{pmatrix}$.
$\pow{\lambda}{2} -4\lambda + 3 = 0$.
$\sub{\lambda}{1} = 1$, $\sub{\lambda}{2} = 3$.
Таким образом, $M\br{0; 0}$ -- <<узел>>, для написанной выше системы неустойчивый.
Для $\sub{\lambda}{1} = 1$ собственный вектор $\vv{v}_{1} = \cbr{1; 1}$
отвечает касательной $y = x$;
для $\sub{\lambda}{2} = 3$ собственным вектором будет
$\vv{v}_{2} = \cbr{1; 3}$, и он задаёт трансверсаль $y = 3x$.

Уравнения траекторий имеют вид $y - x = C\br{y - 3x}^3$
(в них включена $y = x$, но не $y = 3x$),
либо же в параметрической форме
$\tsys{
    & x\br{\tau} = -\tau + C \pow{\tau}{3} \\
    & y\br{\tau} = -\tau + 3C \pow{\tau}{3}
}$.
