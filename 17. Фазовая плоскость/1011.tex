...
Для $\lambda_{1} = 1$ собственный вектор $\vv{v}_{1} = \cbr{1; 1}$ 
отвечает касательной $y = x$;
для $\lambda_{2} = 3$ собственным вектором будет 
$\vv{v}_{2} = \cbr{1; 3}$, и он задаёт трансверсаль $y = 3x$.

Уравнения траекторий имеют вид $y - x = C\br{y - 3x}^3$
(в неё не входит $y = 3x$, но не $y = x$),
либо же в параметрической форме $\tsys{& x\br{\tau} = -\tau + C \pow{\tau}{3} \\& y\br{\tau} = -\tau + 3C \pow{\tau}{3}}$.
