\documentclass[a5paper,10pt]{article}

% https://github.com/InDevRus/filippov-solutions

\input{../preambles/general}
\input{../preambles/plotting}

\begin{document}

$z\`\br{t} = \pow{10}{x + z}$.
$\pow{10}{-z} \cdot z\` = \pow{10}{x}$.

Взяв интеграл левой и правой частей по $\di x$, получаем 
$-\dfrac {\pow{10}{-z}}{\ln 10} = \dfrac{\pow{10}{x}} {\ln 10} + C$.

Окончательно, общее решение равняется
$z\br{x} = -\lg\br{C - \pow{10}{x}}$, где
$C > 0$. Область определения: $x \in \br{-\infty; \lg\br{C}}$.

\begin{figure}[!h]
    \centering
    \begin{tikzpicture}
        \pgfplotsset{
            Graph/.style = {
                samples = 200,
            },
        }
        \begin{axis}[
            width = 12cm,
            ylabel = $z$,
            ymin = -4.2,
            ymax = 4.2,
            xmin = -5.2,
            xmax = 5.2, 
            xtick = {-10, ..., 10},
            ytick = {-10, ..., 10},
            minor tick num = 3,
            declare function = {
                f(\C, \x) = -log10(\C - 10^(x));
                g(\C, \x) = log10(\C - 10^(-x));
            }]
            
            \draw[thick, blue] (-5.5, 0) -- (5.5, 0);
            
            \pgfplotsinvokeforeach {
                0.001, 0.003, 0.01, 0.03, 0.1, 0.25,
                0.5, 1, 3, 11, 32, 100, 300, 1000, 3000, 10000
            } {
                \addplot[Graph, domain = -5.5 : log10(#1 / 2)]
                {f(#1, x)};
                \addplot[Graph, domain = -log10(#1 / 2) : 5.5]
                ({g(#1, x)}, {x});
            }
        \end{axis}
    \end{tikzpicture}
\end{figure}

\end{document}
