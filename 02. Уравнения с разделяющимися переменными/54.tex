...
$y\` \ctg x + y = 2$. $y\` \ctg x + \br{\y - 2} = 0$. $y = 2$
является решением. $\dfrac {y\`} {y - 2} + \tg x = 0$.

\begin{align*}
    \tint \dfrac {y\`} {y - 2} \di x 
    = \tint \dfrac {\di y} {y - 2} 
    = \ln\vbr{\y - 2} + C;
    && \tint \tg x \di x
    = -\ln\vbr{\cos x} + C.
...
$\ln \vbr{\y - 2} - \ln \vbr{\cos x} = C$. 
$y = C \cos x + 2$. 
При этом $y = 2$ получается при $C = 2$.
В итоге общее решение имеет вид $y = C \cos x + 2$.
...
Найдём решение такое, что 
$\tlim{x \to 0} y\br{x} = -1$. 
Подставив в общее решение, получаем 
$-1 = C \cos 0 + 2$. $C = -3$,
и искомое частное решение равно $y = -3 \cos x + 2$.
