...
$\dfrac {\pow{x}{2} y\'_{x}\br{x} - 2x y\br{x}} {\pow{x}{4}} = 0$.
$\dfrac {d} {dx} \br{\dfrac {y\br{x}} {\pow{x}{2}}} = 0$.
$y = C\pow{x}{2}$. Итак, траекториями системы являются параболы, проходящие через начало координат фазовой плоскости.
...
Докажем по определению неустойчивость. Общее решение имеет вид $X\br{t} = \begin{pmatrix} A \pow{e}{t} \\ B \pow{e}{2t} \end{pmatrix}$. Для $t \ge t_{0} = 0$
\begin{align*}
\br{X\br{t} - \Phi\br{t}}^2 = \pow{A}{2} \pow{e}{2t} + \pow{B}{2} \pow{e}{4t} &&
\br{X\br{t_{0}} - \Phi\br{t_{0}}}^2 = \pow{A}{2} + \pow{B}{2}
\end{align*}

Для $\epsilon = 1$ и любого $\delta > 0$ потребуем, чтобы $A$ и $B$ были таковы, что $0 < \pow{A}{2} + \pow{B}{2} < \pow{\delta}{2}$. Обе функции $\pow{e}{2t}$ и $\pow{e}{4t}$ возрастают, то есть при $t \ge t_{0} = 0$ будет $\pow{e}{4t} \ge \pow{e}{2t}$. Однако, при
$t \ge \max\br{ \dfrac {1} {2} \ln\br{\dfrac {1}{\pow{A}{2} + \pow{B}{2}}}; 0}$ имеем
$\pow{A}{2} \pow{e}{2t} + \pow{B}{2} \pow{e}{4t} \ge \pow{e}{2t} \br{\pow{A}{2} + \pow{B}{2}} \ge 1 = \pow{\epsilon}{2}$, и поэтому \linebreak решение неустойчиво.
