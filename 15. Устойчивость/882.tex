...
Докажем по определению устойчивость. Общее решение имеет вид $X\br{t} = \begin{pmatrix} A \pow{e}{-t} \\ B \pow{e}{-2t} \end{pmatrix}$. Для $t \ge t_{0} = 0$
\begin{gather*}
\br{X\br{t} - \Phi\br{t}}^2 = \pow{A}{2} \pow{e}{-2t} + \pow{B}{2} \pow{e}{-4t} \\
\br{X\br{t_{0}} - \Phi\br{t_{0}}}^2 = \pow{A}{2} + \pow{B}{2}
\end{gather*}

Для $\epsilon > 0$ положим $\delta\br{\epsilon} = \epsilon$ и потребуем, чтобы $A$ и $B$ были таковы, что $\pow{A}{2} + \pow{B}{2} < \delt\pow{a}{2}$. Поскольку обе функции $\pow{e}{-2t}$ и $\pow{e}{-4t}$ убывают, $\pow{A}{2} \pow{e}{-2t} + \pow{B}{2} \pow{e}{-4t} \le \pow{A}{2} + \pow{B}{2} < \pow{\delta}{2} = \pow{\epsilon}{2}$, и устойчивость показана.

Асимптотическая устойчивость вытекает из того факта, что
$$\lim_{t \to +\infty} \pow{e}{-2t} = \lim_{t \to +\infty} \pow{e}{-4t} = 0.$$
