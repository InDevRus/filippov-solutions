...
Для $\epsilon > 0$ положим $\delta\br{\epsilon} = \epsilon$ и потребуем, чтобы $A$ и $B$ были таковы, что $\pow{A}{2} + \pow{B}{2} < \delt\pow{a}{2}$. Поскольку обе функции $e^{-2t}$ и $e^{-4t}$ убывают, $\pow{A}{2} e^{-2t} + \pow{B}{2} e^{-4t} \le \pow{A}{2} + \pow{B}{2} < \pow{\delta}{2} = \pow{\epsilon}{2}$, и устойчивость показана.
