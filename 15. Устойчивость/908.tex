...
Для случая $a = -1$ покажем, что нулевое решение неустойчиво с помощью функции Ляпунова. Рассмотрим функцию $v\br{x; y} = e^{x + y} - 1$ в открытой области $V = \cbr{x + y > 0}$. На границе $\partial V = \cbr{x + y = 0}$ имеем $v\br{x; y} = 0$, внутри области $v\br{x; y} > 0$, но $\dfrac {dv} {dt}\bigg|_{a = -1} \linebreak = e^{x + y} \br{\pow{x}{2} + y - x} + e^{x + y} \br{\pow{y}{2} - y + x} = e^{x + y} \br{\pow{x}{2} + \pow{y}{2}} > 0$, так как точка $\br{0; 0}$ не лежит в $V$. По теореме Четаева нулевое решение неустойчиво.
